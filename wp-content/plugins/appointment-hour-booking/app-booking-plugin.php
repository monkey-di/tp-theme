...
/**
// Not longer needed. Temporary added to avoid a conflict with Elementor
add_action( 'init', function(){     
    $allow_resources = function($allowed_tags, $context){         
        $all_attributes = array( 'id' => true, 'class' => true, 'style' => true, 'title' => true, 'role' => true, 'data-*' => true, 'name' => true, 'value' => true, 'type' => true, 'action' => true, 'method' => true, 'placeholder' => true, 'required' => true, 'readonly' => true, 'disabled' => true, 'onclick' => true, 'onsubmit' => true, 'autocomplete' => true, 'enctype' => true, 'step' => true, 'min' => true, 'max' => true, 'pattern' => true, 'size' => true, 'generated' => true, 'rel' => true, 'media' => true, 'href' => true );    
        $allowed_tags['script'] = array( 'type' => true, 'src' => true, 'async' => true, 'defer' => true );
        $allowed_tags['form'] = $all_attributes;
        $allowed_tags['input'] = $all_attributes;
        $allowed_tags['div'] = $all_attributes;
        $allowed_tags['link'] = $all_attributes;
        $allowed_tags['style'] = array('type' => true);
        return $allowed_tags;
    };
    $allow_styles = function( $styles ){
                    $styles[] = 'display';
                    $styles[] = 'position';
                    $styles[] = 'left';
                    $styles[] = 'top';
    ...
    add_action( 'elementor/frontend/widget/before_render', function($element) use ( $allow_resources, $allow_styles ) {
        if ( 'text-editor' == $element->get_name() ) {
            $settings = $element->get_settings();
            if (isset($settings['editor']) && 
                  (strpos($settings['editor'], 'CP_APP_HOUR_BOOKING_LIST') !== false) ||
                  (strpos($settings['editor'], 'CP_APP_HOUR_BOOKING_SCHEDULE_CALENDAR') !== false) ||
                  (strpos($settings['editor'], 'CP_APP_HOUR_BOOKING_LIST_PARAM') !== false) ||
                  (strpos($settings['editor'], 'CP_APP_HOUR_BOOKING') !== false) 
                ) {
                add_filter('wp_kses_allowed_html', $allow_resources, 999, 2);
                add_filter('safe_style_css', $allow_styles, 999);
            }
        }
    });
    add_action( 'elementor/frontend/widget/after_render', function($element) use ( $allow_resources ) {
        remove_filter('wp_kses_allowed_html', $allow_resources);
    });      
} );
